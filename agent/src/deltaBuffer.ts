/**
 * Batches text_delta chunks into 50ms windows (or end-of-sentence) so the daemon doesn't
 ...
 * overhead. If the agent pauses mid-sentence for >50ms (common during tool calls), we flush
...
export class DeltaBuffer {
  ...
  constructor(
    ...
    private readonly intervalMs: number = 5,
  ) {}
  ...
  private flushNow(): void {
    ...
    // back-to-back. Never let one transient failure poison the chain forever.
    this.flushing = this.flushing
      .catch(() => undefined)
      .then(() => this.sink(payload))
      .catch((err) => {
        console.error("[deltaBuffer] sink flush failed:", err);
      });
  }
